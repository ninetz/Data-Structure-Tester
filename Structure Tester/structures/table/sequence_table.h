...
namespace structures
{
	...
	inline T& SequenceTable<K, T>::operator[](const K key)
	{
		...
		if (tableItem != nullptr) {
			...
			throw::std::logic_error("Operator [] sequence table key not found");
		}
	}
	...
	inline void SequenceTable<K, T>::insert(const K& key, const T& data)
	{
		if (!containsKey(key))
		{
			...
			throw::std::logic_error("Operator insert sequence table key exists already");
		}
	}
	...
	inline T SequenceTable<K, T>::remove(const K& key)
	{
		...
		if (tableItem != nullptr) {
			...
			throw::std::logic_error("Operator remove sequence table key doesnt exist");
		}
	}
