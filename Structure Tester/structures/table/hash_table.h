...
namespace structures
{
	...
	inline T& HashTable<K, T>::operator[](const K key)
	{
		...
		{
			throw std::out_of_range("hashtable - Operator [] data not found");
		}
	}
	...
	inline T HashTable<K, T>::remove(const K& key)
	{
		...
		{
			throw std::logic_error("hashtable - remove key not found");
		}
	}
	template<typename K, typename T>
	...
	inline int HashTable<K, T>::hashFunction(const K& key) const
	{
		...
		unsigned long long int nasobok = (key * (key + 3));
		return nasobok % DATA_ARRAY_SIZE;
	}
